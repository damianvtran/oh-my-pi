...
function renderSingleFileResult(
	...
	return framedBlock(uiTheme, width => {
		...
		const headerStats = headerDiff ? getDiffStats(headerDiff) : undefined;
		const statsSuffix = headerStats ? formatChangeStatsSuffix(headerStats.added, headerStats.removed, uiTheme) : "";
		const header = renderEditHeader(width, uiTheme, {
