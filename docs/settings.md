...
| `retry.fallbackCycle`                    | boolean | `true`            | Let an exhausted chain reverse onto the models _before_ the active one instead of dead-ending on its last entry, and let a turn boundary climb partway back while the primary is still capped. Only models whose cooldown already expired qualify; one retry burst may reverse direction at most once; a model that fails again right after recovering has its next cooldown doubled. `false` restores a strictly forward-only walk. Reversal also stands down entirely under `retry.fallbackRevertPolicy: never` and for classifier-refusal fallbacks. |
...
With `retry.fallbackCycle` on, a spent chain reverses instead of stopping: `opus → kimi → codex` returns to `kimi` or `opus` once one of them is out of cooldown. Between turns the session also walks back up one entry at a time, so a primary behind a weekly cap no longer strands it on the last entry — but only after a 60s dwell, only when the entry has not already failed a re-probe, and (with `retry.usageAwareFallback` on) only when that entry's quota still looks usable.
...
compaction:
  ...
  maxThresholdTokens: -1 # ceiling on the resolved threshold when > 0
  remoteEnabled: true
...
| `compaction.maxThresholdTokens` | number  | `-1`          | Ceiling on the resolved threshold when `> 0`, applied after whichever mode above set it, so it only ever compacts _earlier_. Prefer this over `thresholdTokens` to bound session size across mixed context windows: a fixed `thresholdTokens` clamps up to `contextWindow - 1` on smaller windows and delays the trigger, while a ceiling leaves those windows on their reserve-based default. |
