...
### Auth & usage
- **Environment & Wire Credential**: Resolves `ALIBABA_TOKEN_PLAN_API_KEY` then `BAILIAN_TOKEN_PLAN_API_KEY`. Supports plain bearer keys (`sk-sp-...`) or serialized JSON strings (`{ token, cookie?, baseUrl?, oauth? }`) parsed via `parseAlibabaTokenPlanCredential` and formatted via `serializeAlibabaTokenPlanCredential` (`packages/catalog/src/wire/alibaba-token-plan.ts`). A malformed `oauth` member poisons the whole credential rather than silently degrading quota reporting to the console scrape.
- **Interactive Login**: `loginAlibabaTokenPlan` (`packages/ai/src/registry/alibaba-token-plan.ts`) prompts for region (1=International, 2=China Beijing, 3=Custom URL) and validates the API key via `${baseUrl}/models` (`validateApiKeyAgainstModelsEndpoint`). International/custom logins then complete the QwenCloud management-OAuth device flow (PKCE S256, `https://t.qwencloud.com/cli/device/code` → `/cli/device/token`, driven by `pollOAuthDeviceCodeFlow`) and store the grant — `clientId`, `accessToken`, optional `refreshToken`/`expiresAt`/`aliyunId`/`email` — inside the credential; the API key remains the only credential inference ever sees. China (Beijing) logins instead keep the optional `bailian-cs.console.aliyun.com` browser `Cookie` prompt, because the QwenCloud OAuth product does not cover the CN console.
- **Official Quota API (preferred)**: `alibabaTokenPlanUsageProvider` (`packages/ai/src/usage/alibaba-token-plan.ts`) uses the stored OAuth grant to POST flat BSS calls to `https://cli.qwencloud.com/data/v2/api.json` (`Authorization: Bearer`, product `BssOpenAPI-V3`) — the same surface as the first-party `qwencloud-cli`. It probes `QuerySubscriptionGray`: gray accounts report team seats through `GetSeatSubscriptionSummary` (`SubscriptionGroupList[].EquityList` totals, monthly window), everyone else through `DescribeFrInstances` for the `teams`/`personal`/`addon` commodity codes, where the personal commodity is the 7-day credit window (`InitCapacityBaseValue` total, `CurrCapacityBaseValue` remaining, `EndTime` reset) and team seats are monthly (`periodMonthlyShift` capacity). Add-on instances sum into a window-less `credits:packs` limit — Credit Packs draw down outside the subscription window. Expired grants are skipped (falling back to a stored cookie when one exists) until re-login.
- **Console Quota Scraping (China / legacy fallback)**: When no usable OAuth grant exists, the provider scrapes the console data gateway with the stored `Cookie` header: `secToken` from `https://home.qwencloud.com/tool/user/info.json` (or `SEC_TOKEN` scraped from the Beijing console HTML), then POST to the `…/data/api.json?…api=zeldaHttp.apikeyMgr./tokenplan/personal/api/v2/usage` gateway with URL-encoded parameters.
- **Quota Windows & Ranking**: QwenCloud documents no 5-hour Token Plan window (personal = 7-day credits, team = monthly seat credits), so none is synthesized even though the console gateway still returns a `per5HourPercentage`. Limits are `credits:7d` (personal) and `credits:monthly` (team/seat summary) in absolute credits, plus window-less `credits:packs`. `alibabaTokenPlanRankingStrategy` ranks `credits:7d` primary then `credits:monthly`, falling to `credits:packs` as the next constraint (team-only accounts burn `credits:monthly` as primary, so packs is their secondary); `windowDefaults` are 7d / 30d.
